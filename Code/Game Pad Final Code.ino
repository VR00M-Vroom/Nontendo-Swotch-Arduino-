...
void drawPage(int page) {
  ...
  if (page == 0) {
    ...
    u8g2.drawStr(15,50,"By MaDDy Regal");
  } else if (page == 1) {
    u8g2.setFont(u8g2_font_ncenB10_tr);
...
void showGameOverScreen() {
  ...
  while (millis() - t < 3000)
    if (!digitalRead(BUTTON_MENU)) { beep(); delay(200); break; }
}
...
void playCatchGame() {
  ...
  if (ballY >= 60 && ballX >= paddleX && ballX <= paddleX + paddleWidth) {
    ballY = 0; ballX = random(0, SCREEN_WIDTH - 5); score++; beep();
  }
...
void playShooterGame() {
  ...
  if (!digitalRead(BUTTON_SELECT) && !bulletActive) {
    bulletActive = true; bulletX = cannonX + cannonW / 2; bulletY = SCREEN_HEIGHT - cannonH - 2; beep();
  }
  ...
  if (bulletActive && bulletX >= enemyX && bulletX <= enemyX+4 && bulletY <= enemyY+4 && bulletY >= enemyY) {
    shooterScore++; bulletActive = false; enemyY = 0; enemyX = random(0, SCREEN_WIDTH - 5); beep();
  }
  u8g2.clearBuffer();
  char buf[20];
    ...
    for (int c=0; c<wb_cols; c++) {
      if (bricks[r][c]) {
        ...
        if (wb_ballX >= bx && wb_ballX < bx + brickW && wb_ballY >= by && wb_ballY < by + 6) {
          bricks[r][c] = false; wb_ballVY = -wb_ballVY; score++; beep();
        }
      }
    }
  }
...
void playFlappyBird() {
  ...
  if (!digitalRead(BUTTON_SELECT)) { flappyVel = flapStrength; beep(); }
  flappyVel += gravity;
  flappyY += flappyVel;
  pipeX -= 2;
  if (pipeX < -10) { pipeX = SCREEN_WIDTH; gapY = random(10,40); flapScore++; beep(); }
...
void loop() {
  ...
  if (!digitalRead(BUTTON_MENU)) { currentPage = (currentPage+1)%3; lastSwitch = millis(); beep(); delay(200); }
  if (currentPage < 2) {
    ...
    drawPage(2);
    if (!digitalRead(BUTTON_UP))   { menuIndex = (menuIndex+3)%4; beep(); delay(200); }
    if (!digitalRead(BUTTON_DOWN)) { menuIndex = (menuIndex+1)%4; beep(); delay(200); }
